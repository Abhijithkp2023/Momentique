...
import { generateChunkPlanesCached, getChunkUpdateThrottleMs, shouldThrottleUpdate } from "./chunk-utils";
...
function MediaPlane({
  ...
  chunkCy: number;
  chunkCz: number;
  cameraGridRef: React.RefObject<CameraGridState | null>;
}) {
...
function SceneController({
  ...
  const cameraGridRef = React.useRef<CameraGridState | null>({ cx: 0, cy: 0, cz: 0, camZ: camera.position.z });
  ...
  useFrame(() => {
    ...
    const gridRef = cameraGridRef.current;
    if (gridRef) {
      gridRef.cx = cx;
      gridRef.cy = cy;
      gridRef.cz = cz;
      gridRef.camZ = s.basePos.z;
    }
...
export function InfiniteCanvasScene({
  ...
  const isTouchDevice = useIsTouchDevice();
  const dpr = Math.min(typeof window !== "undefined" ? window.devicePixelRatio || 1 : 1, isTouchDevice ? 1.25 : 1.5);
    ...
    <KeyboardControls map={KEYBOARD_MAP}>
      <div className={styles.container}>
        ...
        >
          ...
          {showFps ? <Stats className={styles.stats} /> : null}
        </Canvas>

        {showControls ? (
          <div className={`${styles.controlsPanel} text-xs`}>
            {isTouchDevice ? (
              <div>
                <span className="font-semibold">Drag</span> to pan · <span className="font-semibold">Pinch</span> to zoom
              </div>
            ) : (
              <div>
                <span className="font-semibold">WASD</span> move · <span className="font-semibold">Q / E</span> down / up ·{" "}
